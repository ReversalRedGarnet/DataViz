...
// A reading-progress indicator styled as a canoe sailing along the
// same wave line PacificBorder already uses between sections -- not a
// new motif, the literal same tile geometry (TILE_WIDTH/BASELINE_Y/
// CREST_Y match PacificBorder.jsx exactly), so this reads as part of
// the same visual language rather than a bolted-on progress-bar
// widget. Traveled distance is solid ocean blue; distance still ahead
// is faint ink, the same "muted, not gone" treatment the chart
// gridlines use.
//
// Fixed to the very top of the viewport, full width. There's no
// existing sticky header in App.jsx to collide with.
//
...
export default function ScrollProgress() {
  ...
  // Nothing to draw before the first width measurement -- avoids a
  // one-frame flash of a wave sized for width 0 on first mount.
  if (!width) return null

  const wavePoints = buildWavePoints(width)
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none fixed inset-x-0 top-0 z-40" style={{ height: BAR_HEIGHT }}>
      <svg width={width} height={BAR_HEIGHT} viewBox={`0 0 ${width} ${BAR_HEIGHT}`}>
