...
export default function MapView({ selected, onToggle, onClear }) {
  ...
  // colour updates happen in the effect below so panning/zooming isn't
  // reset every time a marker is clicked.
  useEffect(() => {
    ...
    // longitude (e.g. Fiji at +178 vs Samoa at -172) render on opposite
    // edges of the map instead of near each other.
    ...
    // Drag-to-pan and touch pinch-to-zoom stay on. Mouse-wheel / trackpad
    // scroll is deliberately excluded from triggering zoom (see the
    // filter below) so scrolling PAST the map on the page doesn't
    // accidentally zoom it -- zooming only ever happens via touch pinch
    ...
    // pointer-events is set explicitly so it's clickable despite being
    // transparent.
    marker
      .append('circle')
    ...
    <Section tone="ocean">
      ...
      <div className="relative">
        <svg
          ...
          className="w-full h-auto border rounded-lg bg-white/40"
        />
